{
  ...
  config = {
    ...
    services.pipewire.wireplumber.configPackages = let
      ...
      # Pre‑converted dB → linear (10 ** (dB / 20))
      db = {
        "-36" = 1.5848931924611134e-2;
        "-18.1" = 0.1244514611771385;
        "-12.0" = 0.2511886431509580;
        "-8.0" = 0.3981071705534972;
        "-5.48" = 0.5321082592667942; # retained for completeness
        "-4.76" = 0.5780960474057181; # retained for completeness
        "-2.0" = 0.7943282347242815;
        "-1.5" = 0.8413951416451951;
        "-1.0" = 0.8912509381337456;
        "0.0" = 1.0;
        "1.5" = 1.4125375446227544;
      ...
      # ───────────────────────────────────────────────
      #  Filter‑chain JSON (post‑gain stage + EQ tweak)
      # ───────────────────────────────────────────────
      filter-chain = json.generate "filter-chain.json" {
        ...
        "filter.graph" = {
          nodes = [
            ...
            # 8‑band parametric EQ – gains adjusted for fuller speech
            {
              type = "lv2";
            ...
            # Post‑gain stage
            {
              ...
              name = "post_gain";
              control = {
                gain = db."8.1"; # +8 dB default, user‑adjustable
              };
            }
          ];
          ...
          # Final outputs
          outputs = ["post_gain:out_l" "post_gain:out_r"];

          # Desktop volume → post_gain
          "capture.volumes" = [
            {
