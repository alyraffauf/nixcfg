#!/bin/bash
# Installation script for ThinkPad X1 Carbon Gen 7 audio DSP on non-NixOS distros
...
echo "=== ThinkPad X1 Carbon Gen 7 Audio DSP ==="
echo ""
...
echo ""
echo "Step 3: Installing x1c-gen7-filter-chain.json..."
sudo cp "$SCRIPT_DIR/x1c-gen7-filter-chain.json" /etc/wireplumber/wireplumber.conf.d/x1c-gen7-filter-chain.json

# Substitute placeholders in x1c-gen7-filter-chain.json
sudo sed -i "s|@DEVICE_NAME@|$DEVICE_NAME|g" /etc/wireplumber/wireplumber.conf.d/x1c-gen7-filter-chain.json
echo "✓ Installed x1c-gen7-filter-chain.json with device: $DEVICE_NAME"
...
sudo sed -i "s|@FILTER_PATH@|/etc/wireplumber/wireplumber.conf.d/x1c-gen7-filter-chain.json|g" /etc/wireplumber/wireplumber.conf.d/99-x1c.conf
...
echo "  - /etc/wireplumber/wireplumber.conf.d/x1c-gen7-filter-chain.json"
echo "  - /etc/wireplumber/wireplumber.conf.d/99-x1c.conf"
echo ""
echo "To uninstall: sudo rm /etc/wireplumber/wireplumber.conf.d/{x1c-gen7-filter-chain.json,99-x1c.conf}"
