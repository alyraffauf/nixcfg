...
def get_outputs # rubocop:todo Metrics/AbcSize, Naming/AccessorMethodName
  # rubocop:enable Naming/AccessorMethodName
  hyprctl = IO.popen(%w[hyprctl monitors]).read
...
loop do # rubocop:todo Metrics/BlockLength
  ...
  active_monitors.each do |monitor|
    next unless Time.now - last_update_time[monitor] >= update_interval
